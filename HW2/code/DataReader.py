...
def prepare_X(raw_X):
	...
	### YOUR CODE HERE
    # Flip horizontally, where column0 is now column 15.
    flipped_image = np.fliplr(raw_image) # Shape: [n_samples, 16, 16]
    symmetry_field = -np.sum(np.abs(raw_image - flipped_image), axis=(1,2)) / 256 # Shape: [n_samples,]
	### END YOUR CODE

	# Feature 2: Measure of Intensity
	### YOUR CODE HERE
    intensity_field = np.sum(raw_image, axis=(1,2)) / 256 # Shape: [n_samples,]
	### END YOUR CODE

	# Feature 3: Bias Term. Always 1.
	### YOUR CODE HERE
    bias = np.ones(len(raw_image))
	### END YOUR CODE
